...
- **CI gate per phase**: Set up a CI pipeline (e.g. GitHub Actions) early in the migration — before the first phase merges. Each phase's PR requires its own test subset to pass before merging, preventing partially-migrated states from accumulating on the main branch.
